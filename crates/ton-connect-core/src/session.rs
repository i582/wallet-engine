...
mod tests {
    ...
    /// Ported from the official TypeScript protocol
    /// `session-crypto.spec.ts` at
    /// `beb31b373e0d9db4b7d0bfd55a1ab0d0a439b74a`.
    #[test]
    fn typescript_session_crypto_cases_round_trip() -> Result<(), Box<dyn Error>> {
        let first = SessionCrypto::generate()?;
        let second = SessionCrypto::generate()?;
        ...
        assert_eq!(second.decrypt(first.client_id(), &encrypted)?, plaintext);
        Ok(())
    }
    ...
    /// Generated by `XaBbl4/pytonconnect` `SessionCrypto` at
    /// `15ec6997be196ff63677e3ccc5720f058cda3d98` with fixed keys and nonce.
    #[test]
    fn encryption_matches_python_session_crypto_vector() -> Result<(), Box<dyn Error>> {
        let alice = SessionCrypto::from_secret_key(ALICE_SECRET);
        let bob = SessionCrypto::from_secret_key(BOB_SECRET);
        let plaintext = b"some protocol message: { method_params: [1, 2, null] }";
        let expected = STANDARD.decode(concat!(
            "aWlu6VW2K3PNYr2odfxz1oIZ4ANregs3C5To1mTCuJA5314JDiX0Q0PxCT9UmZLJ",
            "ee0gw7U3F9BpaIrqPxV9CyxkWSo96jPJVVIJssYSp0EMUfTlgAiINWhiklOY2g=="
        ))?;
        assert_eq!(
            alice.encrypt_with_nonce(bob.client_id(), plaintext, NONCE)?,
            expected
        );
        assert_eq!(bob.decrypt(alice.client_id(), &expected)?, plaintext);
        Ok(())
    }
