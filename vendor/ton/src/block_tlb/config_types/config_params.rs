...
impl TLB for ConfigParams {
    fn read_definition(parser: &mut CellParser) -> Result<Self, TonCoreError> {
        ...
        let config = TLBHashMap::<DictKeyAdapterUint<_>, DictValAdapterTLB<_>>::new(32)
            .read(&mut config_ref.parser())?;
        Ok(Self {
            config_addr,
            config,
            ..Default::default()
    ...
    fn write_definition(&self, dst: &mut CellBuilder) -> Result<(), TonCoreError> {
        ...
        TLBHashMap::<DictKeyAdapterUint<_>, DictValAdapterTLB<_>>::new(32)
            .write(&mut config_cell, &self.config)?;
