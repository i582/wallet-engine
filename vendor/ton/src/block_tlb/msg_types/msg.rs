        ...
        match &self.info {
            CommonMsgInfo::ExtIn(_) => {
                let mut msg_normalized = Msg {
                    ...
                    body: TLBEitherRef::new_with_layout(
                        self.body.value.to_cell()?,
                        self.body.layout,
                    ),
                };
    ...
    fn test_ext_in_msg_hash_normalized() -> anyhow::Result<()> {
        let msg_info = CommonMsgInfo::ExtIn(CommonMsgInfoExtIn {
            ...
            dst: MsgAddressInt::Std(MsgAddressIntStd {
                ...
                workchain: -1,
                address: TonHash::from_str(
                    "adfd5f1d28db13e50591d5c76a976c15d8ab6cad90554748ab254871390d9334",
                )?,
            }),
        ...
        assert_eq!(
            hash_norm,
            TonHash::from_str("dfacc0b48826e33a5a127ee1def710a449d8ce79def7c19f43e57b7996e870df")?
        );
