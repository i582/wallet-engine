...
impl Debug for ShardPfx {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "ShardPfx(value: 0x{:016X}, bits_len: {})",
            self.value, self.bits_len
        )
    }
}
...
impl ShardIdent {
    ...
    pub fn split(&self) -> Result<(ShardIdent, ShardIdent), TonCoreError> {
        ...
        if lower_bits & (!0 >> (TON_MAX_SPLIT_DEPTH + 1)) != 0 {
            bail_ton_core!(
                "Can't split shard {}, because of max split depth is {TON_MAX_SPLIT_DEPTH}",
                self.shard
            );
        }
        Ok((
    ...
    pub fn merge(&self) -> Result<ShardIdent, TonCoreError> {
        ...
        Ok(ShardIdent::new(
            self.workchain,
            (self.shard - lower_bits) | (lower_bits << 1),
        ))
    }
...
impl Display for ShardIdent {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "ShardIdent(wc: {}, shard: 0x{:016X})",
            self.workchain, self.shard
        )
    }
}
...
mod tests {
    ...
    fn test_shard_ident_contains_addr() -> anyhow::Result<()> {
        ...
        let (left, right) = shard_ident.split()?;
        assert!(
            !left.contains_addr(&addr.to_msg_address_int()),
            "shard: {left}, addr: {}",
            addr.to_hex()
        );
        assert!(
            right.contains_addr(&addr.to_msg_address_int()),
            "shard: {right}, addr: {}",
            addr.to_hex()
        );
        Ok(())
