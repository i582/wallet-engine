...
from_tvm_stack_primitives_impl!(
...
impl FromTVMStack for TonAddress {
    fn from_stack(stack: &mut TVMStack) -> TonResult<Self> {
        Ok(TonAddress::from_cell(&stack.pop_cell()?)?)
    }
}
...
mod tests {
    ...
    fn test_from_tvm_stack_derive_macro_works() -> anyhow::Result<()> {
        ...
        stack.push_cell(
            TonAddress::from_str("EQBiMfDMivebQb052Z6yR3jHrmwNhw1kQ5bcAUOBYsK_VPuK")?.to_cell()?,
        );
        ...
        assert_eq!(
            test_struct.field2,
            TonAddress::from_str("EQBiMfDMivebQb052Z6yR3jHrmwNhw1kQ5bcAUOBYsK_VPuK")?
        );
