...
macro_rules! extract_stack_val {
    ($maybe_result:expr, $variant:ident) => {
        match $maybe_result {
            ...
            Some(rest) => Err(TonError::TVMStackWrongType(
...
impl TVMStack {
    ...
    pub fn push_cell(&mut self, value: TonCell) {
        self.push(TVMStackValue::Cell(TVMCell {
            value: value.into(),
        }));
    }
    ...
    pub fn pop_num(&mut self) -> TonResult<I512> {
        match self.pop_checked()? {
            ...
            other => Err(TonError::TVMStackWrongType(
                String::from("TinyInt or Int type"),
                other.to_string(),
            ))?,
        }
    }
    ...
    pub fn pop_cell(&mut self) -> TonResult<TonCell> {
        match self.pop() {
            ...
            Some(other) => Err(TonError::TVMStackWrongType(
                "Cell".to_string(),
                format!("{other:?}"),
            )),
        }
...
mod tests {
    ...
    fn test_vm_stack_complex_with_tupl() -> anyhow::Result<()> {
        ...
        assert_eq!(stack, stack_parsed_back);
