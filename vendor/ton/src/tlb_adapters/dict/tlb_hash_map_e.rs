    ...
    KA::KeyType: Eq + Hash + Ord,
{
    ...
    ) -> Result<HashMap<KA::KeyType, VA::ValType>, TonCoreError> {
        if !parser.read_bit()? {
...
mod tests {
    ...
    fn test_dict_blockchain_data() -> anyhow::Result<()> {
        ...
        let parsed_data = TLBHashMapE::<DictKeyAdapterUint<_>, DictValAdapterNum<_, 150>>::new(8)
            .read(&mut parser)?;
        assert_eq!(expected_data, parsed_data);
        let mut builder = TonCell::builder();
        builder.write_bits(&some_data, 96)?;
        TLBHashMapE::<DictKeyAdapterUint<_>, DictValAdapterNum<_, 150>>::new(8)
            .write(&mut builder, &expected_data)?;
