...
pub(super) fn read_up_to_4_msgs(
    parser: &mut CellParser,
) -> Result<(Vec<u8>, Vec<TonCell>), TonCoreError> {
    ...
    for _ in 0..msgs_cnt {
        ...
        msgs.push(parser.read_next_ref()?.to_owned());
    }
    Ok((msgs_modes, msgs))
}
...
impl TLB for InnerRequest {
    ...
    fn write_definition(&self, builder: &mut CellBuilder) -> Result<(), TonCoreError> {
        ...
        if let Some(actions) = &self.out_actions {
            builder.write_ref(actions.to_cell()?)?;
        }
...
pub(super) fn parse_inner_request(
    request: InnerRequest,
) -> Result<(Vec<TonCell>, Vec<u8>), TonCoreError> {
    ...
    for action in out_list.actions {
        if let OutAction::SendMsg(action_send_msg) = &action {
            msgs.push(action_send_msg.out_msg.deref().clone());
...
pub(super) fn build_inner_request(
    msgs: &[TonCell],
    msgs_modes: &[u8],
) -> Result<InnerRequest, TonCoreError> {
    if msgs.is_empty() {
...
mod test {
    ...
    fn test_write_up_to_4_msgs() -> anyhow::Result<()> {
        ...
        let msgs = vec![TonCell::empty().to_owned(), TonCell::empty().to_owned()];
        let msgs_modes = vec![1, 2];
        assert_ok!(write_up_to_4_msgs(&mut builder, &msgs, &msgs_modes));

        let mut builder = TonCell::builder();
        let msgs = vec![TonCell::empty().to_owned(); 4];
